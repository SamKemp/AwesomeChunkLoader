...
public final class H2DataStore implements IDataStore {
    ...
    public boolean load() {
        ...
        try (Connection connection = getConnection()) {
            connection.createStatement().executeUpdate("CREATE TABLE IF NOT EXISTS " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders ("
                    ...
                    + ")");
            connection.createStatement().executeUpdate("CREATE TABLE IF NOT EXISTS " + plugin.getConfig().getCore().dataStore.h2.prefix + "playerdata ("
    ...
    public List<ChunkLoader> getChunkLoaders() {
        ...
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders");
            while (rs.next()) {
    ...
    public List<ChunkLoader> getChunkLoaders(World world) {
        List<ChunkLoader> clList = new ArrayList<>();
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE world = '" + world.getUniqueId().toString() + "'");
            while (rs.next()) {
                Optional<Vector3i> optLocation = serializer.deserialize(rs.getString("location"));
                Optional<Vector3i> optVector = serializer.deserialize(rs.getString("chunk"));
                if (optLocation.isPresent() && optVector.isPresent()) {
    ...
    public List<ChunkLoader> getChunkLoadersByType(Boolean isAlwaysOn) {
        List<ChunkLoader> clList = new ArrayList<>();
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE alwaysOn = '" + isAlwaysOn + "'");
            while (rs.next()) {
                Optional<Vector3i> optLocation = serializer.deserialize(rs.getString("location"));
                Optional<Vector3i> optVector = serializer.deserialize(rs.getString("chunk"));
                if (optLocation.isPresent() && optVector.isPresent()) {
    ...
    public List<ChunkLoader> getChunkLoadersByOwner(UUID ownerUUID) {
        List<ChunkLoader> clList = new ArrayList<>();
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE owner = '" + ownerUUID.toString() + "'");
            while (rs.next()) {
                Optional<Vector3i> optLocation = serializer.deserialize(rs.getString("location"));
                Optional<Vector3i> optVector = serializer.deserialize(rs.getString("chunk"));
                if (optLocation.isPresent() && optVector.isPresent()) {
    ...
    public boolean addChunkLoader(ChunkLoader chunkLoader) {
        try (Connection connection = getConnection()) {
            ...
            if (locationStr.isPresent() && vectorStr.isPresent()) {
                PreparedStatement statement = connection.prepareStatement("INSERT INTO " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public boolean removeChunkLoader(ChunkLoader chunkLoader) {
        try (Connection connection = getConnection()) {
            return connection.createStatement().executeUpdate("DELETE FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE uuid = '" + chunkLoader.getUniqueId() + "' LIMIT 1") > 0;
        } catch (SQLException ex) {
    ...
    public boolean updateChunkLoader(ChunkLoader chunkLoader) {
        try (Connection connection = getConnection()) {
            ...
            Optional<String> vectorStr = serializer.serialize(chunkLoader.getChunk());
            if (locationStr.isPresent() && vectorStr.isPresent()) {
                PreparedStatement statement = connection.prepareStatement("MERGE INTO " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                statement.setString(1, chunkLoader.getUniqueId().toString());
                statement.setString(2, chunkLoader.getWorld().toString());
                statement.setString(3, chunkLoader.getOwner().toString());
                statement.setObject(4, locationStr.get());
    ...
    @Override
    public boolean removeAllChunkLoaders(UUID owner) {
        try (Connection connection = getConnection()) {
            return connection.createStatement().executeUpdate("DELETE FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE owner = '" + owner.toString() + "'") > 0;
        } catch (SQLException ex) {
    ...
    public boolean removeAllChunkLoaders(World world) {
        try (Connection connection = getConnection()) {
            return connection.createStatement().executeUpdate("DELETE FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "chunkloaders WHERE world = '" + world.getUniqueId().toString() + "'") > 0;
        } catch (SQLException ex) {
            plugin.getLogger().error("H2: Error removing ChunkLoaders.", ex);
        }
        return false;
    ...
    public Optional<PlayerData> getPlayerData(UUID playerUUID) {
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "playerdata WHERE uuid = '" + playerUUID.toString() + "' LIMIT 1");
            if (rs.next()) {
    ...
    public Optional<PlayerData> getPlayerData(String playerName) {
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "playerdata WHERE username = '" + playerName + "' LIMIT 1");
            if (rs.next()) {
                return Optional.ofNullable(new PlayerData(
                        rs.getString("username"),
                        UUID.fromString(rs.getString("uuid")),
    ...
    public boolean updatePlayerData(PlayerData playerData) {
        try (Connection connection = getConnection()) {
            PreparedStatement statement = connection.prepareStatement("MERGE INTO " + plugin.getConfig().getCore().dataStore.h2.prefix + "playerdata VALUES (?, ?, ?, ?, ?)");
    ...
    public List<PlayerData> getPlayersData() {
        ...
        try (Connection connection = getConnection()) {
            ResultSet rs = connection.createStatement().executeQuery("SELECT * FROM " + plugin.getConfig().getCore().dataStore.h2.prefix + "playerdata");
            while (rs.next()) {
